...
//#define spi_readwrite bcm2835_spi_transfer()
#define spi_read() spi_readwrite(0x00)
#define endTransaction() bcm2835_spi_end()
...
void MCP_CAN::mcp2515_reset(void)                                      
{
    ...
    delay(10);
    bcm2835_spi_transfer(MCP_RESET);
...
void MCP_CAN::mcp2515_setRegister(const uint8_t address, const uint8_t value)
{
    bcm2835_gpio_write(RPI_GPIO_P1_18, LOW);
    delay(10);
    bcm2835_spi_transfer(MCP_WRITE);
    bcm2835_spi_transfer(address);
...
void MCP_CAN::mcp2515_modifyRegister(const uint8_t address, const uint8_t mask, const uint8_t data)
{
    bcm2835_gpio_write(RPI_GPIO_P1_18, LOW);
    delay(10);
    bcm2835_spi_transfer(MCP_BITMOD);
    bcm2835_spi_transfer(address);
...
uint8_t MCP_CAN::mcp2515_readStatus(void)                             
{
    ...
    bcm2835_gpio_write(RPI_GPIO_P1_18, LOW);
    delay(10);
    bcm2835_spi_transfer(MCP_READ_STATUS);
    i = bcm2835_spi_transfer(0x00);
    bcm2835_gpio_write(RPI_GPIO_P1_18, HIGH);
    delayMicroseconds(10);
    return i;
}
...
*********************************************************************************************************/
uint8_t MCP_CAN::readMsgBuf4(uint32_t *id, uint8_t *ext, uint8_t *len, uint8_t buf[])
{
